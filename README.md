...
For rate-limiting at the auth backend level you can use for example https://pypi.org/project/django-ratelimit-backend/ However, the latest release is from Aug 27th in 2018. There might be a a risk of another flaw from OWASP Top Ten List, that is A06:2021 – Vulnerable and Outdated Components.
...
Security Misconfigurations refer to situations where the system has unnecessary, out-dated or otherwise non-secure features enabled. Users might have more privileges than they need. The system might not have the latest security features enabled or securely configured. Default passwords might be left unchanged.

The poll app has at least two flaws related to misconfiguration. First, all users are created as superusers. This gives them access to the Django admin with unlimited rights. 
...
You should have create_user() instead of create_superuser(). Basic users should have their own user group with restricted, proper rights. 

The feature suggesting username as a nickname should be removed. Maybe you could ask for a nickname already in sign-up and make sure that a user can't select her username as a nickname. 
...
The poll app has a feedback form with xss vulnerability. The product owner wanted to enable bolded text and hyperlinks in feedback messages. Developers solved the need by ignoring validation and adding a safe tag to the feedback message list in the frontpage. Users are now able to bold text and add links, but the app is also vulnerable for attacks. Users can add for example javascript code into the feedback messages. The code is run everytime when any user loads the frontpage where the recent feedback messages are shown.

### How to fix

First, you remove safe tag and add a proper validator in the server-side. Then you either stop supporting bolded text and links or implement a library supporting rich text. Ensuring the user-provided links are safe is tricky. Perhaps you could make a validator that permits only certain domains. Django has many built-in validators to utilize.
...
Security Logging and Monitoring means for example logging all auditable events like logins, failed logins and transactions. In a non-secure system you do not log and monitor security related events. Log messages can be unclear. There can be either too little or too much and irrelevant messages. Without automated alerts many cases can go unnoticed.

In the poll app there is no logging implemented.
